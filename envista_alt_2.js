...
app.controller('MapController', ['$scope', '$http', 'leafletData', function($scope, $http, leafletData) {
  ...
  function addGeoJsonLayerWithClustering(data) {
      //$('body').addClass('loader');
        //  $(".spinner").remove(); // turn loading spinner on
      var markers = L.markerClusterGroup();
      var geoJsonLayer = L.geoJson(data, {
          pointToLayer: function(feature, latlng) {
             var smallIcon = new L.Icon({
                    iconSize:    [30, 30], // 34, 34, size of the icon
                    iconAnchor:  [15, 29], // 12, 30/ 18, 32 point of the icon which will correspond to marker's location
                    popupAnchor: [1, -32], // 4, -30, point from which the popup should open relative to the iconAnchor
      ...
      //$(".spinner").remove(); // turn loading spinner off
  }
